...
class ToyTunGateway:
    def __init__(self, listen: Tuple[str, int], tun_name: str, mtu: int, verbose: bool = False) -> None:
    ...
    def setup_tun_device(self) -> None:
        ...
        if self.mtu:
            try:
                ...
                self.log("Warning: 'ip' command not found — skipping MTU configuration.")
            except subprocess.CalledProcessError as exc:
                self.log(f"Warning: failed to set MTU: {exc}")
    ...
    def on_udp_readable(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                self.log(f"Invalid frame from {addr}: {exc}")
                continue
    ...
    def handle_data_from_udp(self, payload: bytes) -> None:
        ...
        try:
            ...
            self.log(f"Failed to write to TUN: {exc}")
            return
    ...
    def forward_packet_to_clients(self, packet: bytes) -> None:
        ...
        try:
            ...
            self.log(f"Failed to encode packet for UDP: {exc}")
            return
        for addr in active_clients:
            try:
                ...
                self.log(f"Failed to send to {addr}: {exc}")
    ...
    def send_udp_frame(self, frame_type: int, payload: bytes, addr: ClientAddr) -> None:
        ...
        try:
            ...
            self.log(f"Failed to send frame to {addr}: {exc}")
        except ValueError as exc:
            self.log(f"Failed to encode frame: {exc}")
...
def main() -> None:
    ...
    gateway = ToyTunGateway(args.listen, args.tun, args.mtu, verbose=args.verbose)
